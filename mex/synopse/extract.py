...
def extract_variables() -> Generator[SynopseVariable, None, None]:
    ...
    Settings:
        variablenuebersicht_path: Path to the `variablenuebersicht` file,
    ...
    yield from parse_csv(
        settings.variablenuebersicht_path,
        SynopseVariable,
    )
...
def extract_study_data() -> Generator[SynopseStudy, None, None]:
    ...
    Settings:
        metadaten_zu_datensaetzen_path: Path to the `metadaten_zu_datensaetzen` file,
                                        absolute or relative to `assets_dir`
    ...
    """
    settings = SynopseSettings.get()
    yield from parse_csv(settings.metadaten_zu_datensaetzen_path, SynopseStudy)
...
def extract_projects() -> Generator[SynopseProject, None, None]:
    ...
    Settings:
        projekt_und_studienverwaltung_path: Path to the `projekt_und_studienverwaltung`
    ...
    settings = SynopseSettings.get()
    yield from parse_csv(
        settings.projekt_und_studienverwaltung_path,
        SynopseProject,
    )
...
def extract_study_overviews() -> Generator[SynopseStudyOverview, None, None]:
    ...
    Settings:
        datensatzuebersicht_path: Path to the `datensatzuebersicht` file,
                                  absolute or relative to `assets_dir`
    ...
    """
    settings = SynopseSettings.get()
    yield from parse_csv(settings.datensatzuebersicht_path, SynopseStudyOverview)
